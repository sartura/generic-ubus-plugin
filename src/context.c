 ...
 * @return error code.
 */
int context_set_subscription(context_t *context,
                             sr_subscription_ctx_t *subscription) {
  int rc = SR_ERR_OK;
  CHECK_NULL_MSG(context, &rc, cleanup, "input argument context is null");
  CHECK_NULL_MSG(subscription, &rc, cleanup,
                 "input argument subscription is null");
 ...
 * @return error code.
 */
int context_set_startup_connection(context_t *context,
 ...
 * @return error code.
 */
int context_set_ubus_object_filter_file_name(context_t *context,
 ...
 * @return error code.
 */
int context_get_subscription(context_t *context,
                             sr_subscription_ctx_t **subscription) {
  int rc = SR_ERR_OK;
  CHECK_NULL_MSG(context, &rc, cleanup, "input argument context is null");
  CHECK_NULL_MSG(context->subscription, &rc, cleanup,
 ...
 * @return error code.
 */
int context_get_startup_session(context_t *context,
                                sr_session_ctx_t **session) {
  int rc = SR_ERR_OK;
  CHECK_NULL_MSG(context, &rc, cleanup, "input argument context is null");
  CHECK_NULL_MSG(context->startup_session, &rc, cleanup,
                 "context startup_session is null");
 ...
 * @return error code.
 */
int context_get_startup_connection(context_t *context,
                                   sr_conn_ctx_t **connection) {
  int rc = SR_ERR_OK;
  CHECK_NULL_MSG(context, &rc, cleanup, "input argument context is null");
  CHECK_NULL_MSG(context->startup_connection, &rc, cleanup,
                 "context connection is null");
 ...
 * @return error code.
 */
int context_get_ubus_object(context_t *context, ubus_object_t **ubus_object,
                            const char *ubus_object_name) {
  int rc = SR_ERR_OK;
  CHECK_NULL_MSG(context, &rc, cleanup, "input argument context is null");
  ...
  list_for_each_entry(ubus_object_local, &context->ubus_object_list, list) {
    if (strncmp(ubus_object_local->name, ubus_object_name,
                strlen(ubus_object_local->name)) == 0) {
...
int context_add_ubus_object(context_t *context, ubus_object_t *ubus_object) {
  int rc = SR_ERR_OK;
  CHECK_NULL_MSG(context, &rc, cleanup, "input argument context is null");
  CHECK_NULL_MSG(ubus_object, &rc, cleanup,
                 "input argument ubus_object is null");
 ...
 * @return error code.
 */
int context_delete_ubus_object(context_t *context,
                               const char *ubus_object_name) {
  int rc = SR_ERR_OK;
  CHECK_NULL_MSG(context, &rc, cleanup, "input argument context is null");
  CHECK_NULL_MSG(ubus_object_name, &rc, cleanup,
...
int context_delete_all_ubus_object(context_t *context) {
  ...
  list_for_each_entry_safe(ubus_object_p, ubus_object_n,
                           &context->ubus_object_list, list) {
 ...
 * @return error code.
 */
int context_filter_ubus_object(context_t *context, const char *ubus_object_name,
                               bool *skip) {
  int rc = SR_ERR_OK;
  CHECK_NULL_MSG(context, &rc, cleanup, "input argument context is null");
  CHECK_NULL_MSG(ubus_object_name, &rc, cleanup,
  ...
  *skip = false;
  ...
  if (context->ubus_object_filter_file_name != NULL) {
    ...
    if (result != 0) {
      rc = SR_ERR_OK;
      WRN("file %s does not exist, no filtering will be done",
          context->ubus_object_filter_file_name);
      return rc;
    }
