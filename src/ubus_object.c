...
int ubus_object_create(ubus_object_t **ubus_object) {
  ...
  *ubus_object = calloc(1, sizeof(ubus_object_t));
  CHECK_NULL_MSG(*ubus_object, &rc, cleanup,
                 "return argument ubus_object is null");
...
int ubus_object_set_yang_module(ubus_object_t *ubus_object,
  ...
  CHECK_NULL_MSG(ubus_object, &rc, cleanup,
                 "input argument ubus_object is null");
  CHECK_NULL_MSG(yang_module, &rc, cleanup,
                 "input argument yang_module is null");

  yang_module_local = calloc(strlen(yang_module) + 1, sizeof(char));
  CHECK_NULL_MSG(yang_module_local, &rc, cleanup,
                 "memory allocation for yang_module failed");
...
int ubus_object_state_data_subscribe(
  ...
  int rc = SR_ERR_OK;
  CHECK_NULL_MSG(private_ctx, &rc, cleanup,
  ...
  char xpath[256 + 1] = {0};
  snprintf(xpath, strlen(ubus_object->yang_module) + 4, "/%s:*",
           ubus_object->yang_module);

  INF_MSG("Subscribing to operational");
  rc = sr_oper_get_items_subscribe(session, ubus_object->yang_module, xpath, f,
...
int ubus_object_delete_method(ubus_object_t *ubus_object,
  ...
  CHECK_NULL_MSG(ubus_object, &rc, cleanup,
                 "input argument ubus_object is null");
  CHECK_NULL_MSG(method_name, &rc, cleanup,
                 "input argument method_name is null");
...
int ubus_object_delete_all_methods(ubus_object_t *ubus_object) {
  ...
  list_for_each_entry_safe(ubus_method_p, ubus_method_n,
                           &ubus_object->ubus_method_list, list) {
 ...
 * @return error code.
 */
int ubus_object_get_yang_module(ubus_object_t *ubus_object,
                                char **yang_module) {
  int rc = SR_ERR_OK;
  CHECK_NULL_MSG(ubus_object, &rc, cleanup,
                 "input argument ubus_object is null");
  CHECK_NULL_MSG(ubus_object->yang_module, &rc, cleanup,
                 "ubus_object yang_module is null");
...
int ubus_object_get_method(ubus_object_t *ubus_object,
  ...
  list_for_each_entry(ubus_method_local, &ubus_object->ubus_method_list, list) {
    if (strncmp(ubus_method_local->name, method_name,
                strlen(ubus_method_local->name)) == 0) {
